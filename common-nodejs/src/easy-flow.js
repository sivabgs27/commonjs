...
var EasyFlow = (function () {
    ...
    EasyFlow.prototype.trigger = function (event, context) {
        if (this.event(context, event)) {
            if (context.finished) {
                logger_1.log.warn('Warning: trying to exit final state %s', context.state);
                return;
            }
            this.leave(context);
            ...
            logger_1.log.warn('Warning: invalid event triggered %s in state %s', event, context.state);
        }
    };
    ...
    EasyFlow.prototype.dumpTransitions = function () {
        this.resolvedTransitions.forEach(function (t) {
            logger_1.log.warn("%s => (%s) => %s%s", t.from, t.event, t.to, t.final ? ' (final)' : '');
        });
    };
    EasyFlow.prototype.event = function (context, event) {
        ...
        if (matchedTransition) {
            if (this.debug) {
                logger_1.log.warn('Debug: triggered event %s', event);
            }
    ...
    EasyFlow.prototype.enter = function (context, nextState) {
        ...
        runAsync(function () {
            ...
            if (_this.debug) {
                logger_1.log.warn('Debug: entered state %s', nextState);
            }
            if (context.nextStateFinal) {
                context.finished = true;
                if (_this.debug) {
                    logger_1.log.warn('Debug: context finished');
                }
            }
...
function responseHandler(context, trigger, hint, events) {
    return function (err, res, body) {
        ...
        if (err) {
            logger_1.log.warn('Error handled by responseHandler while %s: %s', hint, err.message);
        }
